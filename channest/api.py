...
def calculate_channel_parameters(
    settings: Union[Dict[str, Any], str], output_directory: str
):
    """
    ...
    **data_file**. In addition to these settings, advanced settings are described below. There are several available
    ...
    if cropbox is None:
        ...
        box = polygonize.Box(
            cropbox["x_0"], cropbox["y_0"], cropbox["x_1"], cropbox["y_1"]
        )
    ...
    for s in setups:
        et = (
            0.5 / s.merge_layers if s.element_threshold is None else s.element_threshold
        )
        ...
        if _cache_state in _cache:
            ...
            polys = polygonize.calculate_polygons(
                cube, slicer, et, s.alpha_hull, minimum_polygon_area
            )
            poly_skels = [
                skeletonize.SkeletonizedLayer(h.polygons)
                for h in progress(polys, "Skeletonizing layers")
            ]
        ...
        poly_widths = [
            ...
            for _p, _sp in progress(
                zip(polys, poly_skels), desc="Calculating widths", total=len(polys)
            )
        ]
        ...
        poly_heights = [
            ...
            for _pw, _dc in progress(
                zip(poly_widths, vdist_cube[slicer]),
                desc="Calculating thicknesses",
                total=len(poly_widths),
            )
        ]
    ...
    for (
        ...
        create_width_height_scatter(
            f"{output_directory}/tw_scatter",
            poly_widths,
            poly_heights,
            dxy,
            scatter_max_height,
        ...
        if generate_plots is True:
            lwp.add_widths(
                poly_widths, [p.map2d for p in polys], 0.5 / key.merge_layers, post_fix
            )
            lhp.add_heights(poly_heights, post_fix)
            sliceplot.create_slice_plot(
                f"{output_directory}/slices{post_fix}",
                list(zip(polys, poly_skels, poly_widths)),
            )
            sliceplot.create_heights_slice_plot(
                f"{output_directory}/heights{post_fix}", poly_heights
            )
            sliceplot.create_kde_slice_plot(
                f"{output_directory}/kde{post_fix}", poly_heights
            )
            stacking.create_stacking_plot(
                f"{output_directory}/stacking{post_fix}", poly_heights
            )
        if generate_fences is True:
            fence_dir = f"{output_directory}/fences"
            os.makedirs(fence_dir, exist_ok=True)
            fences.generate_fences(
                fence_dir, poly_skels, grid_params.box.x0, grid_params.box.y0, dxy
            )
