...
def create_kde_slice_plot(fn: str, data: List[LayerHeights]):
    ...
    for d in data:
        ...
        f.add_scatter(
            x=x, y=y, visible=visible, name="Density Estimate", fill="tozeroy"
        )
        f.add_histogram(
            x=v,
            histnorm="",
        ...
        f.add_scatter(
            ...
            visible=visible,
            mode="markers",
            marker=dict(symbol="line-ns", line=dict(width=1, color="#c00"), size=24),
            name="Data Points",
            secondary_y=True,
        )
        cumulative.append(len(f.data))
    ...
    f.update_yaxes(
        title_text="Histogram (Count)", secondary_y=True, rangemode="nonnegative"
    )
...
def create_slice_plot(
    fn: str, data: List[Tuple[HulledPolygons, SkeletonizedLayer, LayerWidths]]
):
...
def create_heights_slice_plot(fn: str, data: List[LayerHeights]):
    ...
    cs = (
        ...
        + [[v, c[:-1].replace("rgb", "rgba") + ",1.0)"] for v, c in orig_cs[1:]]
    )
...
def _create_slider(cumulative_data):
    ...
    for i in range(len(cumulative_data)):
        s = dict(method="restyle", args=["visible", [False] * cumulative_data[-1]])
