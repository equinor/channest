...
class LayerHeightPlot:
    ...
    def add_heights(self, heights: List[LayerHeights], suffix: str):
        # Calculate statistics: mean, median, max, modes
        values = [h.flat_values() for h in heights]
        values = [
            v if v.size > 0 else np.zeros(1) for v in values
        ]  # Transform to make stats calculations easier
        ...
        self._f.add_scatter(
            x=means, y=ys, mode="lines+markers", name="Mean      " + suffix
        )
        self._f.add_scatter(
            x=medians, y=ys, mode="lines+markers", name="Median    " + suffix
        )
        self._f.add_scatter(
            x=maxs,
            y=ys,
        ...
        for i_layer, layer in enumerate(heights):
            ...
            x_points = np.insert(
                kde_modes[0], [0, kde_modes[0].size], [0, maxs[i_layer]]
            )
            ...
            self._f.add_scatter(
                ...
                text=[
                    f"Layer #: {i_layer}<br>Peak: {p:2.2f}"
